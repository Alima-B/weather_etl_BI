...
class GoldPipeline:
    ...
    def aggregate_weather_daily(self):
        ...
        v4 : 
        hourly_breakdown : add hour_utc, hour_local (hour), pressure,wind gust, and direction, heat warning,weather severity
        """
        ...
        # ici pour ne récupérer que les silver du jour précédant et actuel (pas tout les fichiers à traité)
        for file in self.silver_base_path.rglob("weather_*_cleaned.json"):
            try:
                with open(file) as f:
                    record = json.load(f)
                    # Remove the forecast from treatment. keeped in private key
                    forecast = record.pop("forecast",[])
                    record["_forecast"] = forecast
        ...
        for (city, date), records in weather_data.items():
            try:
                # Sort by hour
                records.sort(key=lambda r: r.get("hour_utc", 0))
                ...
                heat_warning = [r["heat_index_warning"] for r in records if r.get("heat_index_warning")]
                uvi_categories = [r["uvi_category"] for r in records if r.get("uvi_category")]
                
                # Hourly breakdown : add pressure,wind gust, and direction, heat warning,weather severity
                hourly_breakdown = []
                for r in records:
                    hourly_breakdown.append({
                        "hour_utc": r.get("hour_utc"),
                        "hour_utc_formatted": r.get("hour_utc_formatted"),
                        "hour": r.get("hour"),
                ...
                # keep forecast from the last hour recorded
                last_forecast = records[-1].get("_forecast", [])        
                ...
                #     logger.warning(f"No forecast data in last record for {city} on {date}")
                ...
                gold_record = {
                    ...
                    "hours_covered": [r.get("hour_utc") for r in records if r.get("hour_utc") is not None],
                    ...
                    "max_heat_warning" : max(heat_warning, key=lambda s: ["safe", "caution", "extreme caution", "danger","extreme danger"].index(s)) if heat_warning else "unknown",
    ...
    def aggregate_combined_daily(self):
        ...
        # Load weather gold data (change to open only path from today)
        weather_gold = {}
        ...
        for key in common_keys:
            ...
            try:
                ...
                combined_record = {
                    ...
                    "avg_feels_like" : weather.get("avg_feels_like"),
                    "avg_humidity": weather.get("avg_humidity"),
                    "avg_wind_speed" :weather.get("avg_wind_speed"),
                    ...
                    "avg_uvi": weather.get("avg_uvi"),
                    "avg_pressure": weather.get("avg_pressure"),
                    "dominant_weather_condition" : weather.get("dominant_weather_condition"),
                    
                    # Air quality summary
                    "avg_aqi": air.get("avg_aqi"),
                    "max_aqi": air.get("max_aqi"),
                    "aqi_trend": air.get("aqi_trend"),
                    "avg_pm25" : air.get("avg_pm25"),
                    "avg_pm10" : air.get("avg_pm10"),
                    "avg_no2" : air.get("avg_no2"),
                    "avg_o3" : air.get("avg_o3"),
                    "avg_co" : air.get("avg_co"),
                    "avg_so2" : air.get("avg_so2"),
