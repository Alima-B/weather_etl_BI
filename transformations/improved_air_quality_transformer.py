...
v3.1 : 
- Extract UTC and Local (Paris) hours from _storage.hour_timestamp.
- Extract local date (for forecast coherence)
"""
...
class AirQualityTransformer:
    ...
    @staticmethod
    def _extract_hour(raw_record: Dict[str, Any]) -> Optional[Dict[str, int]]:
        """Extract hour from _storage.hour_timestamp.
        v3.1 : Extract UTC and Local (Paris) hours from _storage.hour_timestamp.
        """
    ...
    def transform(raw_record: Dict[str, Any], city_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if timestamp is None and "s" in time_info:
            try:
                ...
                local_time = datetime.strptime(time_str, "%Y-%m-%d %H:%M:%S").isoformat()
        ...
        # Extract hour from _storage (UTC and local)
        hours = AirQualityTransformer._extract_hour(raw_record)
        hour_utc = hours["dt"]
        hour = hours["dt_local"]
