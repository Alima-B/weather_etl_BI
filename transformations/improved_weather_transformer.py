...
class WeatherTransformer:
    ...
    def transform(raw_record: Dict[str, Any], city_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        v4 : 
        - Transform current weather and forecast (nested data)
        - Add local hour
        ...
        # Separate first element (weather at the time) and remaining (forecast)
        ...
        # Weather info (array with one element)
            ...
            # Timestamp from hourly.dt (Unix timestamp and local France)
            dt_value = item_hourly.get("dt")
            if dt_value:
                timestamp_utc = datetime.fromtimestamp(int(dt_value), tz=timezone.utc).isoformat()
                timestamp_local = datetime.fromtimestamp(int(dt_value), tz=timezone.utc).astimezone(ZoneInfo("Europe/Paris")).isoformat()
            else:
                timestamp_utc = datetime.now(timezone.utc).isoformat()
                timestamp_local = datetime.now(ZoneInfo("Europe/Paris")).isoformat()
                logger.warning("No dt found in hourly data, using current time")
            
            # Extract hour (UTC and local) and local date from _storage for consistency
            hour_UTC = datetime.fromisoformat(timestamp_utc).hour
            ...
            visibility = item_hourly.get("visibility")
            pop = item_hourly.get("pop", 0)
            ...
            transformed_item = {
                ...
                "hour_utc": hour_UTC,
                "hour_utc_formatted": f"{hour_UTC:02d}:00" if hour_UTC is not None else None,
                "hour" : hour,
                ...
                # Derived metrics
                "heat_index_celsius": WeatherTransformer._calculate_heat_index(temp or 0, humidity or 0)["hi"],
                "heat_index_warning": WeatherTransformer._calculate_heat_index(temp or 0, humidity or 0)["warning"],
