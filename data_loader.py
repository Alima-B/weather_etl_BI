...
def local_date_to_utc_bounds(local_date_str: str, timezone: str = "Europe/Paris") -> dict:
    """
...
@st.cache_data(ttl=300)
def load_hourly_data(collection :str ,forecast :False ,date: Optional[str] = None, date_start :Optional[str] = None
    ...
    # Index-friendly date filter (operates on UTC dates)
    if date:
        bounds = local_date_to_utc_bounds(date, timezone)
        # UTC range may span two calendar dates (e.g. Paris UTC+2)
        if bounds["start_date"] == bounds["end_date"]:
            date_filter = bounds["start_date"]
        else:
            date_filter = {"$gte": bounds["start_date"], "$lte": bounds["end_date"]}
        hour_filter = _build_hour_filter(bounds)

    elif date_start and date_end:
        # Convert both local bounds to UTC
        b_start = local_date_to_utc_bounds(date_start, timezone)
        b_end   = local_date_to_utc_bounds(date_end, timezone)
        date_filter = {"$gte": b_start["start_date"], "$lte": b_end["end_date"]}
        hour_filter = {
            "$or": [
                # First day: only hours from the local start time onward
                {"date": b_start["start_date"], "hour_utc": {"$gte": b_start["start_hour"]}},
                # Intermediate days: all hours are valid
                {"date": {"$gt": b_start["start_date"], "$lt": b_end["end_date"]}},
                # Last day: only hours strictly before the local end time
                {"date": b_end["end_date"], "hour_utc": {"$lt": b_end["end_hour"]}}
            ]
        }

    elif date_start:
        b_start = local_date_to_utc_bounds(date_start, timezone)
        date_filter = {"$gte": b_start["start_date"]}
        hour_filter = {
            "$or": [
                # Start date: only hours from the local start time onward
                {"date": b_start["start_date"], "hour_utc": {"$gte": b_start["start_hour"]}},
                # All subsequent dates: no hour restriction
                {"date": {"$gt": b_start["start_date"]}}
            ]
        }

    elif date_end:
        b_end = local_date_to_utc_bounds(date_end, timezone)
        date_filter = {"$lte": b_end["end_date"]}
        hour_filter = {
            "$or": [
                # All dates before the end date: no hour restriction
                {"date": {"$lt": b_end["end_date"]}},
                # End date: only hours strictly before the local end time
                {"date": b_end["end_date"], "hour_utc": {"$lt": b_end["end_hour"]}}
            ]
        }

    else:
        raise ValueError("At least one date parameter is required: date, date_start, or date_end")
...
@st.cache_data(ttl=300)
def load_hourly_period(city : Optional[str], date = None ,d_start =None, d_end =None) -> pd.DataFrame:
    '''
    ...
    if date is None : 
        df_air   = load_hourly_data("gold_air_quality_daily",forecast = False, date=None,
                                    date_start=d_start, date_end=d_end, city=city)
        df_meteo = load_hourly_data("gold_weather_daily",forecast = False, date=None,
                                    date_start=d_start, date_end=d_end, city=city)
    else : 
        df_air   = load_hourly_data("gold_air_quality_daily",forecast =False, date=date,
                                    date_start=None, date_end=None, city=city)
        df_meteo = load_hourly_data("gold_weather_daily",forecast =False,  date=date,
                                    date_start=None, date_end=None, city=city)
